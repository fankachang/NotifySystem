...
管理員需要能夠透過 Web 後台介面管理使用者、群組、訊息類型、設定群組的接收規則（時段、聚合、重複告警等）、查看發送歷史、以及產生統計報表。

**Why this priority**: 後台管理功能讓管理員能夠有效維運系統。由於所有設定都由管理員控制，後台管理介面的重要性大幅提升。

**Independent Test**: 可以透過管理員登入後台、建立新群組、設定群組的接收時段和聚合規則、將使用者加入群組、查看過去 7 天的發送統計報表來驗證管理功能完整性。
...
4. **Given** 管理員設定群組啟用聚合發送（5 分鐘），**When** 5 分鐘內有 10 則 CRITICAL 告警，**Then** 群組成員只收到一則聚合訊息
5. **Given** 管理員查看統計報表，**When** 選擇過去 30 天的週報，**Then** 顯示每日發送量趨勢圖、各類型訊息分布、發送成功率、以及尖峰時段分析
...
- **當系統在尖峰時段（如多台主機同時故障）收到大量訊息時**：系統應使用訊息佇列進行緩衝，確保不會因為瞬間流量過大而遺失訊息或導致 API 超時。
...
- **當資料庫連線失敗時**：系統應回應 503 Service Unavailable，並記錄錯誤日誌。重要的是，不應遺失佇列中的訊息。
...
- **FR-023**: 管理員必須能夠為群組啟用聚合發送功能，並設定聚合間隔（預設為 5 分鐘）
- **FR-024**: 管理員必須能夠為群組設定是否接收重複告警，並設定重複告警間隔（預設為 30 分鐘）
- **FR-025**: 一位使用者可以同時屬於多個群組

#### 訊息類型管理

- **FR-026**: 系統必須提供預設的訊息類型：CRITICAL、WARNING、UNKNOWN、OK、INFO
- **FR-027**: 管理員必須能夠新增自訂訊息類型，包括代碼、名稱、說明、優先級、顏色、圖示
- **FR-028**: 管理員必須能夠修改自訂訊息類型的屬性
- **FR-029**: 管理員必須能夠停用或啟用訊息類型，停用後該類型的訊息不會被發送
- **FR-030**: 管理員必須能夠刪除自訂訊息類型，但系統預設類型不可刪除
- **FR-031**: 系統必須根據訊息類型的優先級進行排序顯示（1 為最高，5 為最低）

#### 訂閱管理（由管理員透過群組完全控制）

- **FR-032**: 使用者的訊息訂閱必須完全由管理員透過群組設定來決定，使用者無法自行訂閱或取消訂閱訊息類型
- **FR-033**: 當管理員將使用者加入群組時，系統必須自動為該使用者建立對應的訂閱（根據群組關聯的訊息類型）
- **FR-034**: 當管理員將使用者從群組移除時，系統必須自動移除該使用者對應的訂閱
- **FR-035**: 使用者必須能夠查看自己所屬的群組以及透過群組訂閱的訊息類型（唯讀，無法修改）
- **FR-036**: 當群組的訊息類型關聯變更時，系統必須自動更新所有群組成員的訂閱

#### 訊息發送

- **FR-037**: 系統必須提供 RESTful API 端點 `POST /api/v1/messages/send` 供外部系統發送訊息
- **FR-038**: API 必須使用 Bearer Token（API Key）進行認證
- **FR-039**: API 必須接受 JSON 格式的訊息內容，包括 messageType、title、content、source、metadata、targetGroups、priority
- **FR-040**: 系統必須驗證訊息類型是否有效且已啟用
- **FR-041**: 系統必須根據訊息類型查詢所有訂閱該類型的使用者
- **FR-042**: 系統必須套用群組的接收時段設定，在非接收時段內不發送訊息
- **FR-043**: 系統必須套用群組的靜音時段設定，在靜音時段內不發送訊息
- **FR-044**: 系統必須套用群組的來源主機篩選，只發送符合篩選條件的訊息
- **FR-045**: 系統必須套用群組的來源服務篩選，只發送符合篩選條件的訊息
- **FR-046**: 當 API 請求指定 targetGroups 時，系統必須只發送給指定群組的成員
- **FR-047**: 系統必須對同一使用者進行去重處理，即使該使用者符合多個發送條件，也只發送一則訊息
- **FR-048**: 系統必須使用訊息佇列處理訊息發送，避免同步發送造成 API 回應延遲
- **FR-049**: 系統必須在 API 回應中返回訊息 ID、接收者數量、以及成功/失敗狀態
- **FR-050**: 系統必須使用 Line Messaging API 發送訊息到使用者的 Line 帳號
- **FR-051**: 系統必須使用 Flex Message 格式化告警訊息，包含訊息類型圖示、標題、來源資訊、內容、時間戳記
- **FR-052**: 系統必須在訊息發送失敗時啟用重試機制，重試 3 次（間隔 1 秒、5 秒、30 秒）
- **FR-053**: 系統必須記錄每則訊息的發送狀態（pending、sent、failed、skipped）
- **FR-054**: 系統必須記錄發送失敗的錯誤訊息和重試次數

#### 聚合與重複告警處理

- **FR-055**: 當群組啟用聚合發送時，系統必須在設定的時間間隔內收集相同類型的訊息，並聚合為一則訊息發送給該群組成員
- **FR-056**: 聚合訊息必須顯示「收到 N 則 [訊息類型] 告警」以及最新的告警內容
- **FR-057**: 當群組設定不接收重複告警時，系統必須檢查相同來源主機和服務的告警，並在設定的時間間隔內不重複發送給該群組成員

#### 訊息歷史與報表

- **FR-058**: 系統必須記錄所有發送的訊息，包括訊息內容、訊息類型、來源資訊、metadata、發送時間
- **FR-059**: 系統必須記錄每則訊息的所有接收者和發送狀態
- **FR-060**: 使用者必須能夠查詢自己收到的訊息歷史，依時間範圍、訊息類型、發送狀態篩選
- **FR-061**: 管理員必須能夠查詢所有訊息的發送歷史，依時間範圍、訊息類型、來源主機/服務、發送狀態篩選
- **FR-062**: 管理員必須能夠查看統計報表，包括每日/週/月發送量、各類型訊息分布、發送成功率、尖峰時段分析
- **FR-062A**: 系統必須自動刪除超過 90 天的訊息紀錄和發送記錄，以控制資料庫容量

#### API Key 管理

- **FR-063**: 管理員必須能夠建立新的 API Key，並為其命名（如「Nagios Production」）
- **FR-064**: 系統必須產生唯一的 API Key，並只在建立時顯示一次（之後只顯示前綴）
- **FR-065**: 系統必須使用 SHA-256 雜湊儲存 API Key，不儲存明文
- **FR-066**: 系統必須記錄每個 API Key 的最後使用時間
- **FR-067**: 管理員必須能夠撤銷（刪除）API Key，撤銷後該 Key 立即失效
- **FR-068**: 管理員必須能夠查看所有 API Key 的列表，包括名稱、建立時間、最後使用時間、狀態

#### 系統設定與安全

- **FR-069**: 系統必須支援設定 Line Channel Access Token 和 Channel Secret
- **FR-070**: 系統必須實作 Rate Limiting，限制每個 IP 每分鐘最多 100 次 API 請求
- **FR-071**: 系統必須使用 HTTPS 進行所有 API 通訊
- **FR-072**: 系統必須記錄所有敏感操作的審計日誌，包括管理員操作、API Key 建立/撤銷、使用者停用/啟用
...
- **Group（群組）**: 代表使用者的分類群組，是系統的核心管理單位。所有接收設定都在群組層級進行，包括關聯的訊息類型、來源篩選、接收時段、聚合設定、重複告警處理等。關鍵屬性包括群組代碼（唯一）、群組名稱、說明、來源主機篩選、來源服務篩選、接收時段（開始/結束）、靜音時段（開始/結束）、是否啟用聚合、聚合間隔、是否接收重複告警、重複告警間隔、啟用狀態。與 User（多對多）、MessageType（多對多）有關聯。
...
- **SC-009**: 管理員能夠在 3 分鐘內完成建立群組、設定群組的所有接收規則（訊息類型、時段、聚合等）、並加入成員的完整流程
- **SC-010**: 系統可用性必須達到 99.5%（每月允許約 3.6 小時的計畫性或非計畫性停機）
- **SC-011**: 資料庫必須每日自動備份，備份檔案必須保留 30 天
- **SC-012**: 當同一群組在 5 分鐘內收到 10 則相同類型的告警時，透過群組的聚合發送設定，群組成員只收到 1 則訊息（減少 90% 的訊息干擾）
- **SC-013**: 相較於傳統簡訊服務，使用 Line 發送告警的成本必須降低至少 95%（假設簡訊每則 3 元，Line 訊息免費）
- **SC-014**: 系統必須正確處理群組的來源篩選設定，確保群組成員只收到符合篩選條件的告警（篩選準確率 100%）
- **SC-015**: 使用者對告警訊息的回應時間（從收到通知到開始處理問題）必須比傳統簡訊快至少 50%（透過使用者調查或日誌分析驗證）
